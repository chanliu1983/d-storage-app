...
class TokenRegistry {
  ...
  private readonly SOLANA_TOKEN_LIST_URL = 'https://raw.githubusercontent.com/solana-labs/token-list/main/src/tokens/solana.tokenlist.json';
    ...
    try {
      ...
      const tokens: TokenInfo[] = data.tokens
        .filter((token: any) => {
          ...
          return token.chainId === 101 && // Mainnet
                 token.name && 
                 token.symbol && 
                 token.logoURI &&
  ...
  async getTokenMetadata(connection: Connection, mintAddress: string): Promise<TokenInfo | null> {
    // Check cache first
    ...
    try {
      ...
      const knownToken = popularTokens.find(token => token.mint === mintAddress);
      ...
      try {
        const metadataInfo = await this.getMetaplexMetadata(connection, mintPubkey);
        if (metadataInfo) {
  ...
  private async getMetaplexMetadata(connection: Connection, mintPubkey: PublicKey): Promise<TokenMetadata | null> {
    try {
